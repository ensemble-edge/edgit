...
function inferPrefixFromPath(filePath) {
    ...
    if (normalizedPath.includes('logic/') ||
...
export class ComponentsCommand extends Command {
    ...
    async listComponents(registry, flags) {
        ...
        const tagsOnly = flags['tags-only'];
        const tracked = flags.tracked;
        const untracked = flags.untracked;
        ...
        if (componentEntries.length === 0) {
            if (untracked) {
                ...
                this.showInfo('No tracked components found. Add components with "edgit components add <name> <path> <type>".');
            }
            else {
     ...
     */
    async findUntrackedComponents(registry) {
     ...
     */
    async listComponentsTable(componentEntries, flags, limit) {
     ...
     */
    async listComponentsTree(componentEntries, limit) {
        ...
        for (let typeIdx = 0; typeIdx < types.length; typeIdx++) {
            ...
            for (let compIdx = 0; compIdx < components.length; compIdx++) {
                ...
                for (let verIdx = 0; verIdx < limitedVersions.length; verIdx++) {
                    ...
                    const isLastVer = verIdx === limitedVersions.length - 1 && environmentTagsInfo.length === 0;
                    try {
                        ...
                        for (const envTagInfo of environmentTagsInfo) {
                            try {
                                const envSHA = await this.tagManager.getTagSHA(prefix, entityType, name, envTagInfo.slot);
                                if (process.env.DEBUG) {
                                    console.error(`  [DEBUG] Comparing: version ${versionInfo.slot} SHA=${tagInfo.sha} vs ${envTagInfo.slot} SHA=${envSHA}`);
                                }
                        ...
                        console.log(`${prefix_str}${innerPrefix}${isLastVer ? '└─' : '├─'} ${versionInfo.slot} (${sha}, ${date})${deployBadge}`);
                    }
                    catch (error) {
    ...
    async showComponent(registry, componentName, flags) {
        ...
        try {
            ...
            const versionTagsInfo = await this.tagManager.getVersionTags(prefix, entityType, componentName);
            const environmentTagsInfo = await this.tagManager.getEnvironmentTags(prefix, entityType, componentName);
            // Show version tags
            if (versionTagsInfo.length > 0) {
                console.log('\n🏷️  Version Tags:');
                for (const versionInfo of versionTagsInfo) {
                    try {
                        const gitTag = this.tagManager.buildTagPath(prefix, entityType, componentName, versionInfo.slot);
                        const tagInfo = await this.tagManager.getTagInfo(gitTag);
                        console.log(`   ${versionInfo.slot} - ${tagInfo.sha.substring(0, 8)} - ${tagInfo.date} - ${tagInfo.message}`);
                    }
                    catch {
            ...
            if (environmentTagsInfo.length > 0) {
                ...
                for (const envInfo of environmentTagsInfo) {
                    try {
                        const gitTag = this.tagManager.buildTagPath(prefix, entityType, componentName, envInfo.slot);
                        const tagInfo = await this.tagManager.getTagInfo(gitTag);
                        // Find which version this points to
                        let pointsToVersion = 'custom';
                        for (const versionInfo of versionTagsInfo) {
                            try {
                                const versionSHA = await this.tagManager.getTagSHA(prefix, entityType, componentName, versionInfo.slot);
                        ...
                        console.log(`   ${envInfo.slot} → ${pointsToVersion} (${tagInfo.sha.substring(0, 8)}) - ${tagInfo.date}`);
                    }
                    catch {
            ...
            const customTagsInfo = allTagsInfo.filter((tagInfo) => !versionTagsInfo.some((v) => v.slot === tagInfo.slot) &&
                !environmentTagsInfo.some((e) => e.slot === tagInfo.slot));
            if (customTagsInfo.length > 0) {
                console.log('\n🏷️  Custom Tags:');
                for (const customInfo of customTagsInfo) {
                    try {
                        const gitTag = this.tagManager.buildTagPath(prefix, entityType, componentName, customInfo.slot);
                        const tagInfo = await this.tagManager.getTagInfo(gitTag);
                        console.log(`   ${customInfo.slot} - ${tagInfo.sha.substring(0, 8)} - ${tagInfo.date} - ${tagInfo.message}`);
                    }
                    catch {
        ...
        console.log(`   edgit tag set ${componentName} prod v1.0.0 && edgit push --tags --force # Deploy version`);
    }
    /**
    ...
    async checkoutComponent(registry, componentSpec, flags) {
        ...
        try {
            ...
            if (spec.ref) {
                ...
                const entityType = componentTypeToEntityType(component.type);
                const prefix = inferPrefixFromPath(component.path);
                content = await this.tagManager.getFileAtTag(prefix, entityType, spec.name, spec.ref, component.path);
     ...
     */
    async addComponent(registry, name, filePath, type, flags) {
        ...
        // Create component
        const component = ComponentUtils.createComponent(filePath, componentType);
        // Add to registry
    ...
    async showComponentSummary(name, component, verbose, limit) {
        try {
            ...
            const limitedVersionTagsInfo = limit && limit > 0 ? allVersionTagsInfo.slice(-limit) : allVersionTagsInfo;
            ...
            if (verbose) {
                ...
                console.log(`   Versions: ${allVersionTagsInfo.length}${limit ? ` (showing ${limitedVersionTagsInfo.length})` : ''}`);
                if (environmentTagsInfo.length > 0) {
                ...
                console.log(`   📦 ${name}: ${latestVersion} (${allVersionTagsInfo.length} versions${limitInfo})${deployInfo}`);
            }
        }
        catch (error) {
