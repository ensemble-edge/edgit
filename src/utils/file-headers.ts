...
export const DEFAULT_HEADER_FORMATS: Record<string, HeaderFormat> = {
  ...
  '.json': {
    ...
    suffix: '',
    template:
      '"_edgit": {"id": "{componentId}", "version": "{version}", "component": "{component}"}',
  },
}
...
export class FileHeaderManager {
   ...
   */
  async readUserHeader(filePath: string): Promise<UserComponentHeader | null> {
    try {
      const content = await fs.readFile(filePath, 'utf8')
      const lines = content.split('\n').slice(0, 10) // Only check first 10 lines

      const header: UserComponentHeader = {}
  ...
  async writeMetadata(
    ...
    if (ext === '.yaml' || ext === '.yml') {
      ...
      newContent = this.updateCommentMetadata(
        content,
        metadata,
        ext,
        options.replace,
