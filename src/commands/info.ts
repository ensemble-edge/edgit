...
export class InfoCommand extends Command {
   ...
   */
  private async gatherStatus(): Promise<EdgitStatusOutput> {
   ...
   */
  private isEdgitInitialized(): boolean {
   ...
   * Read edgit registry
   */
  private async readRegistry(): Promise<EdgitRegistry | null> {
   ...
   */
  private emptyComponentCounts(): ComponentCounts {
    return {
      total: 0,
      prompts: 0,
      schemas: 0,
   ...
   */
  private countComponents(registry: EdgitRegistry | null): ComponentCounts {
   ...
   */
  private getDeployments(registry: EdgitRegistry | null): DeploymentInfo[] {
   ...
   */
  private getRecentVersions(limit: number = 5): VersionInfo[] {
    ...
    try {
      ...
      for (const tag of tags.slice(0, limit)) {
        ...
        if (parts.length >= 3) {
          ...
          const versionInfo: VersionInfo = { component: name, type, version }
          if (date) versionInfo.date = date
   ...
   */
  private countTotalVersions(): number {
    try {
      const output = execSync(
   ...
   */
  private async countUntrackedComponents(registry: EdgitRegistry | null): Promise<number> {
   ...
   */
  private displayFull(status: EdgitStatusOutput): void {
    console.log('')
    ...
    if (status.componentsCount.total > 0) {
      ...
      if (status.componentsCount.tools > 0) parts.push(`${status.componentsCount.tools} tools`)
      const other =
        status.componentsCount.queries +
    ...
    console.log(
      `  Production:       ${status.stats.prodDeployments > 0 ? colors.success(String(status.stats.prodDeployments)) + ' components' : colors.dim('None')}`
    )
    console.log(
      `  Staging:          ${status.stats.stagingDeployments > 0 ? colors.accent(String(status.stats.stagingDeployments)) + ' components' : colors.dim('None')}`
    )
    console.log('')
   ...
   */
  private displayCompact(status: EdgitStatusOutput): void {
    console.log('')
    console.log(colors.primaryBold('Edgit Status'))
    console.log('')
