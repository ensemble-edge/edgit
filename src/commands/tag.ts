...
export class TagCommand extends Command {
   ...
   */
  private async listComponentTags(componentName: string, registry: ComponentRegistry, standalone: boolean = true): Promise<void> {
    const component = ComponentUtils.findComponentByName(registry, componentName);
    ...
    try {
      ...
      const customTags = allTags.filter(tag => 
        !this.isVersionTag(tag) && !this.isDeploymentTag(tag)
      );
