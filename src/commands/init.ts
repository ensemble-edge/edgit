...
export class InitCommand extends Command {
  ...
  private async addFileHeaders(components: Component[]): Promise<void> {
    ...
    for (const component of components) {
      try {
        ...
        await fileHeaderManager.writeMetadata(
          filePath,
          {
        ...
        this.showWarning(
          `Failed to add header to ${component.path}: ${error instanceof Error ? error.message : 'Unknown error'}`
        )
      }
    }
  }
  ...
  private async createComponentsRegistry(
    ...
    for (const component of components) {
      const componentName = ComponentUtils.generateComponentName(
        component.path,
        component.type,
        registry
      )
...
`
      ...
      console.warn(
        `   ⚠️  Warning: Could not setup environment: ${error instanceof Error ? error.message : 'Unknown error'}`
      )
    }
  }
  ...
  private async ensureInGitignore(repoRoot: string, pattern: string): Promise<void> {
    ...
    try {
      ...
      if (!patternExists) {
        ...
        const newContent =
          gitignoreContent.trim() + (gitignoreContent.trim() ? '\n' : '') + pattern + '\n'
        await fs.writeFile(gitignorePath, newContent, 'utf8')
        console.log(`   • Added ${pattern} to .gitignore`)
      }
    } catch (error) {
      console.warn(
        `   ⚠️  Warning: Could not update .gitignore: ${error instanceof Error ? error.message : 'Unknown error'}`
      )
    }
  }
