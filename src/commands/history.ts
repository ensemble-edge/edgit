...
Examples:
  ...
  private async showAllComponentsHistory(
    registry: ComponentRegistry, 
    options: HistoryOptions
  ): Promise<void> {
    const histories: ComponentHistory[] = [];
  ...
  private async buildComponentHistory(
    component: Component, 
    options: HistoryOptions
  ...
  private async getCommitMessage(commit: string): Promise<string> {
    try {
  ...
  private async getCommitChanges(commit: string, filePath: string): Promise<any> {
    try {
      ...
      if (statLine) {
        ...
        return {
          added: parseInt(added as string) || 0,
          removed: parseInt(removed as string) || 0,
          files: [filePath]
        };
      }
    } catch {
  ...
  private async outputHistory(histories: ComponentHistory[], options: HistoryOptions): Promise<void> {
