...
Examples:
  ...
  private extractDetectOptions(parsed: { flags: Record<string, boolean>; options: Record<string, string>; positional: string[] }): DetectOptions {
    return {
  ...
  private async analyzeFile(
    ...
    let patterns: { matched: string[]; reason: string } = { matched: [], reason: 'No patterns matched' };
  ...
  private getMatchedPatterns(file: string, detector: ComponentDetector): { matched: string[]; reason: string } {
  ...
  private async generateHeaderPreview(filePath: string, name: string, version: string): Promise<string> {
    try {
